...
router.post("/create", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await AdventureStore.createAdventure(req.body).then(result => {
            if (result.code === 200) {
...
router.get("/:userId/discover", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await FilterService.getRecommendationFeed(req.params.userId).then(result => {
            if (result.code === 200) {
                return res.status(200).send(result.payload);
            }
...
router.get("/search-by-title", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await AdventureStore.searchAdventuresByTitle(req.query.title).then(result => {
            if (result.code === 200) {
                return res.status(200).send(result.payload);
            }
...
router.get("/:adventureId/detail", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await AdventureStore.getAdventureDetail(req.params.adventureId).then(result => {
            if (result.code === 200) {
                return res.status(200).send(result.payload);
            }
...
router.put("/:adventureId/update", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await AdventureStore.updateAdventure(req.params.adventureId, req.body).then(result => {
            if (result.code === 200) {
                return res.status(200).send(result.payload);
            }
...
router.put("/:adventureId/cancel", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await AdventureStore.cancelAdventure(req.params.adventureId).then(result => {
            if (result.code === 200) {
                return res.status(200).send(result.payload);
            }
...
router.get("/:userId/get-adventures", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await AdventureStore.getUsersAdventures(req.params.userId).then(result => {
            if (result.code === 200) {
                return res.status(200).send(result.payload);
            }
...
router.put("/:adventureId/quit", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await AdventureStore.removeAdventureParticipant(req.params.adventureId, req.query.userId).then(result => {
...
router.get("/nearby", async (req, res) => {
    if (Session.validSession(req.headers.cookie) || TestMode.on) {
        await FilterService.getNearbyAdventures(req.query.city).then(result => {
            if (result.code === 200) {
                return res.status(200).send(result.payload);
            }
